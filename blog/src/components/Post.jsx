    ...
    return(
        <>
           {postInfo === undefined ? null: (
            <NavLink to={`/blogs/${postInfo.post_id}`} className="flex flex-col gap-5 px-10 py-10 ">
            {feature ? (
            ...
            <div className={`relative group border border-black cursor-pointer ${feature ? 'w-1/3': 'w-full'} `}>
                <div className="overflow-hidden aspect-w-1 aspect-h-1">
                    <img className="object-cover w-full h-full transition-all duration-300 group-hover:scale-125" src={ img || postInfo.img} alt={`${postName} image`} />
                </div>
